 ...
 */
export function normalizeThemePayload(
    data: any,
    fallbackId = "preview-theme",
): Theme {
    ...
    const rawSettings =
        data.settings || data.currentSettings
            ? data.settings || {
    ...
    return {
        ...
        description:
            data.description || "Previewing custom theme configuration.",
 ...
 */
function decodeThemeFromBase64(base64Str: string): any {
    try {
...
export async function parseThemeFromUrl(
    ...
    const rawSettingParam =
        searchParams.get("setting") || searchParams.get("config");
