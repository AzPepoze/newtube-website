<script lang="ts">
    ...
    async function loadReports() {
        ...
        error = "";
        try {
            const response = await fetch(`${PUBLIC_API_URL}/admin/reports`, { credentials: "include" });
            if (response.status === 403) throw new Error("This page is available to marketplace administrators only.");
            if (!response.ok) throw new Error("Unable to load moderation reports.");
            reports = unwrapList(await response.json());
        } catch (cause) {
            error = cause instanceof Error ? cause.message : "Unable to load moderation reports.";
        } finally {
            loading = false;
        }
    }
    ...
    async function updateReport(report: Report, status: Report["status"]) {
        busy = report.id;
        try {
            const response = await fetch(`${PUBLIC_API_URL}/admin/reports/${report.id}`, {
                method: "PUT",
                credentials: "include",
            ...
            reports = reports.map((item) => item.id === report.id ? { ...item, status } : item);
        } catch (cause) {
            error = cause instanceof Error ? cause.message : "Unable to update report.";
        } finally {
            busy = null;
        }
    }
    ...
    async function hideTheme(report: Report) {
        ...
        busy = report.id;
        try {
            const response = await fetch(`${PUBLIC_API_URL}/admin/themes/${themeId}/visibility`, {
                method: "PUT",
                credentials: "include",
                headers: { "Content-Type": "application/json" },
            ...
            error = cause instanceof Error ? cause.message : "Unable to hide theme.";
        } finally {
            busy = null;
        }
    }
        ...
        <div class="queue">
            {#each reports as report (report.id)}
                <article class="report glass-panel">
                    <div class="report-main"><div class="report-title"><h2>{report.theme?.themeName || `Theme ${report.themeId || report.theme?.themeId || ""}`}</h2><span class:open={report.status === "open"} class="status">{report.status}</span></div><dl><div><dt>Reason</dt><dd>{report.reason}</dd></div><div><dt>Reported</dt><dd>{date(report.createdAt)}</dd></div></dl>{#if report.details}<p class="details">{report.details}</p>{/if}{#if report.themeId || report.theme?.themeId}<a href={`/themes/${report.themeId || report.theme?.themeId}`}>Open theme</a>{/if}</div>
                    <div class="actions"><button type="button" disabled={busy === report.id} onclick={() => updateReport(report, "dismissed")}>Dismiss</button><button type="button" disabled={busy === report.id} onclick={() => updateReport(report, "resolved")}>Resolve</button><button class="danger" type="button" disabled={busy === report.id} onclick={() => hideTheme(report)}>Hide theme</button></div>
                </article>
            {/each}
        </div>
