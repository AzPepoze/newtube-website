<script lang="ts">
	...
	let error = $state("");
	let sourceType = $state<"id" | "setting" | "data" | "manual" | "none">("none");

	let viewMode = $state<"watch" | "home" | "channel" | "grid" | "card" | "code">("watch");
	...
	onMount(() => {
		...
		function handleMessage(event: MessageEvent) {
			...
			if (event.data.type === "SET_THEME_PREVIEW" && event.data.theme) {
				theme = normalizeThemePayload(event.data.theme, "postmessage-theme");
				sourceType = "manual";
				rawInputCode = JSON.stringify(theme.settings, null, 2);
			} else if (event.data.type === "SET_THEME_SETTINGS" && event.data.settings) {
				theme = normalizeThemePayload({ settings: event.data.settings }, "postmessage-theme");
				sourceType = "manual";
				rawInputCode = JSON.stringify(theme.settings, null, 2);
			}
		}
...
<svelte:head>
	<title>{theme ? `${theme.themeName} - Theme Preview` : "Theme Preview - NewTube Store"}</title>
</svelte:head>
	...
	<main class="preview-stage">
			...
			{:else if viewMode === "grid"}
				<div class="grid-iframe-stage">
					<div class="stage-card">
						<div class="card-label"><MaterialIcon name="movie" size={16} /> Watch Page</div>
						<YouTubeMockupIframe {theme} pageMode="watch" height="calc(100vh - 110px)" />
					</div>
					<div class="stage-card">
						<div class="card-label"><MaterialIcon name="home" size={16} /> Home Feed Grid</div>
						<YouTubeMockupIframe {theme} pageMode="home" height="calc(100vh - 110px)" />
					</div>
					<div class="stage-card">
						<div class="card-label"><MaterialIcon name="person" size={16} /> Channel Page</div>
						<YouTubeMockupIframe {theme} pageMode="channel" height="calc(100vh - 110px)" />
					</div>
				</div>
		...
		{:else}
			<div class="empty-state">
				...
				<p>{error || "Provide a theme ID via ?id=YOUR_ID or input setting JSON directly."}</p>
				<button class="install-action-btn" onclick={() => (showRawModal = true)}>
